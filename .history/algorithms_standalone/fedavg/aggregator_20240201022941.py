...
class FedAVGAggregator(Aggregator):
    ...
    def select_pluggable(self, test_data, select_index, client_list=None):
        ...
        if select_index=='KL':
            ...
            return np.array(CKA_list), np.argmax(CKA_list)
            ...
            OOD_list=self.cal_OOD(client_list, test_data)
            return np.array(OOD_list), np.argmax(OOD_list)
            ...
            return np.array(MMD_list), np.argmin(MMD_list)
    ...
    def reduce_plugs(self):
        ...
        for idx, train_data in enumerate(self.train_data_local_dict):
            ...
            print("np.setdiff1d(np.where(normed_MMD_list<0.25), [idx])")
            reduced_idx.extend(np.setdiff1d(np.where(normed_MMD_list<0.25), [idx]))