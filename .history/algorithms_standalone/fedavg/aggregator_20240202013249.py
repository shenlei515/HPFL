...
class FedAVGAggregator(Aggregator):
    ...
    def select_pluggable(self, test_data, select_index, client_list=None):
        ...
        if select_index=='KL':
            ...
            if self.args.reduce_plug:
                MMD_list=self.cal_MMD([self.dist_list[i] for i in self.reduce_map], test_data)
            else:
                MMD_list=self.cal_MMD(self.dist_list, test_data)
            score = np.array(MMD_list)
            choice = np.argmin(MMD_list)
        if self.args.reduce_plug:
            return score, self.reduce_map[choice]
        else:
            return score, choice
    ...
    def reduce_plugs(self):
        reduced_idx = set()
        for idx, train_data in self.train_data_local_dict.items():
            if idx not in reduced_idx:
                MMD_list = np.array(self.cal_MMD(self.dist_list, train_data))
                normed_MMD_list = (MMD_list-np.min(MMD_list)) / (np.max(MMD_list) - np.min(MMD_list))
                print("normed_MMD_list", normed_MMD_list)
                print(f"np.where(normed_MMD_list<{self.args.reduce_threshold})", np.where(normed_MMD_list<self.args.reduce_threshold))
                print(f"np.setdiff1d(np.where(normed_MMD_list<{self.args.reduce_threshold}), [idx])", np.setdiff1d(np.where(normed_MMD_list<self.args.threshold), [idx]))
                reduced_idx.update(list(np.setdiff1d(np.where(normed_MMD_list<self.args.reduce_threshold), [idx])))

        print("reduced_idx", reduced_idx)
        self.reduce_map = list(range(len(self.dist_list)))
        for index in sorted(reduced_idx, reverse=True):
            self.reduce_map.pop(index)
        wandb.run.summary['plug_num_after_reduce'] = len(self.reduce_map)
        print("self.reduce_map", self.reduce_map)
        print("len(self.reduce_map)", len(self.reduce_map))
        # exit()
