    ...
    def reduce_plugs(self):
        ...
        for idx, train_data in self.train_data_local_dict.items():
            if idx not in reduced_idx:
                ...
                print("np.where(normed_MMD_list<0.025)", np.where(normed_MMD_list<self.args.threshold0.025))
                print("np.setdiff1d(np.where(normed_MMD_list<0.025), [idx])", np.setdiff1d(np.where(normed_MMD_list<0.025), [idx]))
