    ...
    def reduce_plugs(self):
        ...
        for idx, train_data in self.train_data_local_dict.items():
            if idx not in reduced_idx:
                ...
                print(f"normed_MMD_list", normed_MMD_list)
                print(f"np.where(normed_MMD_list<0.025)", np.where(normed_MMD_list<self.args.threshold))
                print(f"np.setdiff1d(np.where(normed_MMD_list<0.025), [idx])", np.setdiff1d(np.where(normed_MMD_list<self.args.threshold), [idx]))
                reduced_idx.update(list(np.setdiff1d(np.where(normed_MMD_list<self.args.threshold), [idx])))
